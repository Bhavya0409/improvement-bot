...
const archiveTaskCommand = new SlashCommandBuilder()
	...
	.setDescription('Mark a task as archived by adding the "archive" tag')
	.addStringOption(option =>
		option
			.setName('task')
			.setDescription('The task to mark as archived')
			.setRequired(true)
			.setAutocomplete(true)
	);

const archiveTask = async (interaction) => {
	try {
		...
		const tag = await Tag.findOne({ where: { value: 'archive' } });
		if (!tag) {
			return await interaction.reply({
				content: '❌ The "archive" tag does not exist.',
				ephemeral: true,
			});
		}
		...
		if (existing) {
			return await interaction.reply({
				content: `❌ The task '${task.value}' is already marked as archived.`,
				ephemeral: true,
			});
		}
...
export { archiveTaskCommand, archiveTask };
